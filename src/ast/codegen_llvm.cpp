  ...
  if (type == Type::string)
  {
    ...
    switch (binop.op) {
      ...
      case bpftrace::Parser::token::NE:
        ...
        break;
      default:
        std::cerr << "Error: missing codegen to string operator \"" << opstr(binop) << "\"" << std::endl;
        abort();
    }
  ...
  else
  {
    ...
    switch (binop.op) {
      ...
      case bpftrace::Parser::token::LOR:
        ...
        abort();
      default:
        std::cerr << "Error: missing codegen (LLVM) to string operator \"" << opstr(binop) << "\"" << std::endl;
        abort();
    }
  }
...
void CodegenLLVM::visit(Unop &unop)
{
  ...
  if (type.type == Type::integer)
  {
    switch (unop.op) {
      ...
      default:
        std::cerr << "Error: missing codegen to union expression type" << std::endl;
        abort();
    }
  }
  ...
  {
    std::cerr << "Error: missing codegen to union operator \"" << opstr(unop) << "\"" << std::endl;
    abort();
  }
}
